...
export default function OwnerInvoicesPage() {
  ...
  const { user, branches, selectedBranchId } = useAuth();
  ...
  const invoicesQuery = useMemoFirebase(() => {
    ...
    let q = query(collection(firestore, 'invoices'), orderBy('createdAt', 'desc'));
    ...
    return q;
  }, [firestore, user, selectedBranchId]);
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-navy">
              {selectedBranch ? `Invoices: ${selectedBranch.name}` : 'Semua Invoices'}
            </h1>
            <p className="text-gray-500 mt-1">
              {selectedBranch ? 'Menampilkan invoice untuk cabang yang dipilih.' : 'Pilih cabang untuk memfilter data.'}
            </p>
          </div>
        </div>
